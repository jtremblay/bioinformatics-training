...
class Pipeline(object):
    ...
    def argparser(self):
        if not hasattr(self, "_argparser"):
            ...
            self._argparser.add_argument("-j", "--job-scheduler", help="job scheduler type (default: slurm)", choices=["slurm", "torque", "batch", "daemon"], default="slurm")
        ...
        for step in self.step_range:
            ...
            for job in jobs:
                ...
                if(job.job_array_num_task > 0):
                    ...
                    missing_tasks_num = []
                    job.done_job_array = os.path.join("job_output", step.name, job.name + "." + hashlib.md5(job.command).hexdigest() + ".nrc.done")
                    for curr_task_num in range(0,job.job_array_num_task):
                        job.done = os.path.join("job_output", step.name, job.name + "." + hashlib.md5(job.command).hexdigest() + ".nrc.done")
